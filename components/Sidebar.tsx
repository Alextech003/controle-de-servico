...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, currentUser, onLogout, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar - Drawer no Mobile, Fixo no Desktop - LARGURA REDUZIDA EM DESKTOP (w-56) */}
      <aside className={`
        fixed md:static inset-y-0 left-0 z-50 w-72 md:w-56 bg-white border-r border-slate-200 
      ...
      `}>
        ...
        <div className="p-6 md:p-8 flex flex-col items-center">
          <Logo size={56} className="mb-4 shadow-xl shadow-blue-50" />
          <div className="text-center">
              <h1 className="text-lg md:text-xl font-black text-[#0A192F] leading-none tracking-tighter uppercase">
              ...
              <p className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Monitoramento 24H</p>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 md:px-4 space-y-2 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={18} className={isActive ? 'text-[#00AEEF]' : ''} />
        ...
        <div className="p-4 border-t border-slate-100 bg-white">
          <div className="px-4 py-3 bg-slate-50 rounded-2xl mb-3 border border-slate-100">
              <p className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Identificação</p>
              <p className="text-xs md:text-sm font-black text-slate-800 truncate mt-1">{currentUser.name}</p>
